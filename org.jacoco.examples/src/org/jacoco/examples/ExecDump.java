...
public final class ExecDump {
	...
	private void dump(final String file) throws IOException {
		...
		reader.setSessionInfoVisitor(info -> out.printf("Session \"%s\": %s - %s%n", info.getId(), new Date(
                info.getStartTimeStamp()),
                new Date(info.getDumpTimeStamp())));
		reader.setExecutionDataVisitor(data -> out.printf("%016x  %3d of %3d   %s%n",
                data.getId(),
                getHitCount(data.getProbes()),
                data.getProbes().length,
                data.getName()));
		reader.read();
		in.close();
		out.println();
